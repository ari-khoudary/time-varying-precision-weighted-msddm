function data = doSampling_serial(config)
...
if noisePeriods == 1
    ...
    noisePDF = discrete_bounded_hazard_rate(expLambda, noiseMin);
...
if noisePeriods==0
    ...
    noise1max = max(noise1Frames);
    signal1max = max(signal1Frames);
    noise2max = max(noise2Frames);
    minFlicker = noise1max + signal1max + noise2max;
    nFrames = round(minFlicker);
end
...
memoryEvidence = (binornd(1, cue, [max(noise1Frames), nTrial])*2-1) + normrnd(0,1, [max(noise1Frames), nTrial]);
...
for trial=1:nTrial
    ...
    for frame=1:nFrames
        ...
        % get the evidence from pre-generated samples
        if frame < signal1Onsets(trial)
        ...
        % accumulate
        if frame < signal1Onsets(trial)
            if mod(frame, memoryThinning) == 1 % on frames when a memory sample is  drawn
                if frame == 1
                      memoryAccumulator(frame, trial) = memorySample;
                      decisionVariable = memoryAccumulator;
                else
                    memoryAccumulator(frame, trial) = memoryAccumulator(frame-1, trial) + memorySample;
                    decisionVariable(frame, trial) = memoryAccumulator(frame, trial);
                end
            else % on frames when a memory sample isn't drawn
                memoryAccumulator(frame, trial) = memoryAccumulator(frame-1, trial);
                decisionVariable(frame, trial) = memoryAccumulator(frame, trial);
            end
        else % after the onset of visual evidence
            visionAccumulator(frame, trial) = visionAccumulator(frame-1, trial) + visualSample;
            decisionVariable(frame, trial) = decisionVariable(frame-1, trial) + visualSample;
        end
    end
