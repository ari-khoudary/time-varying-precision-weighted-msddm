...
def process_subjects(results_dir, data_dir, plot_results=False, subjects_to_process=None):
    ...
    # Get model name
    model_name = re.search(r'cluster_fitting/([^/]+)/results', results_dir)
    ...
    for cue_dir in cue_dirs:
        ...
        if os.path.exists(cue_path):
            summary_files = glob.glob(os.path.join(cue_path, 'sub*_*coh_summary.txt'))
            all_summary_files.extend([(cue_dir, os.path.basename(f)) for f in summary_files])
    
    # Extract unique subjects
    all_subjects = sorted(set(int(re.match(r'sub(\d+)_', f[1]).group(1)) 
                              for f in all_summary_files if re.match(r'sub(\d+)_', f[1])))
    ...
    for subID in subjects:
        ...
        subject_files = [(cue_dir, fname) for cue_dir, fname in all_summary_files 
                        if fname.startswith(f'sub{subID}_')]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', type=str, default='../../../simulated_data/serial_data/',
