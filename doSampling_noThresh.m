function [choices] = doSampling_noThresh(cueLevel, coherence, congruent, memoryThinning, ...
...
for subj=1:nSub
    for trial=1:nTrial
        ...
        noise1Duration = round(unifrnd(500/2, 1000/2));
        ...
        % use final state of dv to determine choice
...
if saveFiles
    % save workspace
    outfile = sprintf('results_v2/workspace_files/%.2fcue_%icong_noThresh.mat', cueLevel, congruent);
    save(outfile)

    % write CSVs
    behav_csv = table(choices, noise1_duration, noise2_onset, noise2_duration);
    csv_name = sprintf('results_v2/behav_csv/%.2fcue_%icong_noThresh.csv', cueLevel, congruent);
