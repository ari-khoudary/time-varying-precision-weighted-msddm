function [choices, RT] = doSampling_grf(static, cueLevel, coherence, congruent, threshold, memoryThinning, ...
...
for subj=1:nSub
    for trial=1:nTrial
        ...
        % load in visual evidence
        trialcourse = csvread('grf_figs/0.7coh_1cong_vizSamps.csv');
        samples(:, visual_idx, 1) = trialcourse;
        if ~congruent
            samples(:, visual_idx) = -samples(:, visual_idx);
        end
...
%         samples(noise2Onset:noise2Onset+noise2Duration, visual_idx, trial) = normrnd(0,1,[noise2Duration+1,1]);
        ...
        if static
            samples(1:noise1Duration, memory_idx, trial) = (binornd(1,cueLevel, [noise1Duration,1])*2-1) + normrnd(0,1, [noise1Duration,1]);
        else
            ...
            if t == 1
                ...
                if static
                    ...
                    if t < noise1Duration
                        ...
                        evidence(t, memory_idx, trial) = NaN;
                    end
                else
                    evidence(t, visual_idx, trial) = evidence(t-1, visual_idx, trial) + visualSample * visualDriftRate;
                    if ~mod(t, memoryThinning)
...
if saveFiles
    % save workspace
    outfile = sprintf('results_v2/workspace_files/%.2fcue_%icong_%.2fcoh_%ithresh_%ithin.mat', cueLevel, congruent, coherence, threshold, memoryThinning);
    save(outfile)

    % write CSVs
    behav_csv = table(choices, RT, noise1_duration, noise2_onset, noise2_duration);
    csv_name = sprintf('results_v2/behav_csv/%.2fcue_%icong_%.2fcoh_%ithresh_%ithin.csv', cueLevel, congruent, coherence, threshold, memoryThinning);
